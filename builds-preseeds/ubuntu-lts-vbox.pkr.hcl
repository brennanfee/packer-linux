...
build {
  source "sources.virtualbox-iso.ubuntu-preseed" {
    ...
    skip_export     = false
    vm_name         = "bfee-ubuntu-${local.edition}-vbox-barePreseed"
  ...
  post-processor "shell-local" {
    inline = [
      ...
      "mv -f ${path.root}/packer-manifest.json ${path.root}/../images/bfee-ubuntu-${local.edition}-vbox-barePreseed-manifest.json",
    ]
  }
}
