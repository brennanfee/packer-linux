...
build {
  source "sources.virtualbox-iso.ubuntu-preseed" {
    ...
    skip_export     = false
    vm_name         = "bfee-vagrantVbox-ubuntu-${local.edition}-${local.configuration}"
  ...
  post-processor "vagrant" {
    output = "${path.root}/../../images/bfee-vagrantVbox-ubuntu-${local.edition}-${local.configuration}.box"
  }

  post-processor "shell-local" {
    inline = [
      "mv -f ${path.root}/packer-manifest.json ${path.root}/../../images/bfee-vagrantVbox-ubuntu-${local.edition}-${local.configuration}-manifest.json",
    ]
  }
}
