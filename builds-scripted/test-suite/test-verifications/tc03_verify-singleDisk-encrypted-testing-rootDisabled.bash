...
((total_tests=total_tests+1))
result="PASS"
if [[ $(cat /etc/timezone || true) != "America/Los_Angeles" ]]
then
  ((failed_tests=failed_tests+1))
  result="FAIL"
fi
...
((total_tests=total_tests+1))
result="PASS"
if [[ $(lsblk -nd -oNAME,RO | awk '/0$/ {print $1}' | grep -cv '^sr' || true) != "1" ]]
then
  ((failed_tests=failed_tests+1))
  result="FAIL"
fi
...
((total_tests=total_tests+1))
result="PASS"
if [[ $(wc -l /etc/crypttab | cut -d' ' -f 1 || true) != "2" ]]
then
  ((failed_tests=failed_tests+1))
  result="FAIL"
fi
...
((total_tests=total_tests+1))
result="PASS"
if [[ $(passwd -S root | cut -d' ' -f 2 || true) != "L" ]]
then
  ((failed_tests=failed_tests+1))
  result="FAIL"
fi
...
((total_tests=total_tests+1))
result="PASS"
if [[ $(dpkg-query -W -f='${Status}' "ansible" 2>/dev/null | grep -c "ok installed" || true) != "1" ]]
then
  ((failed_tests=failed_tests+1))
  result="FAIL"
fi
