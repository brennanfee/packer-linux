...
((total_tests = total_tests + 1))
result="PASS"
if [[ $(dmsetup deps -o devname cryptroot | sed -n -e 's/.*(\([^)]*\).*/\1/p' || true) != sda* ]]; then
  ((failed_tests = failed_tests + 1))
  result="FAIL"
fi
...
((total_tests = total_tests + 1))
result="PASS"
if [[ $(dmsetup deps -o devname cryptdata | sed -n -e 's/.*(\([^)]*\).*/\1/p' || true) != sdb* ]]; then
  ((failed_tests = failed_tests + 1))
  result="FAIL"
fi
...
((total_tests = total_tests + 1))
result="PASS"
if [[ $(wc -l /srv/after-script.log | cut -d' ' -f1 || true) != "1" ]]; then
  ((failed_tests = failed_tests + 1))
  result="FAIL"
fi
