...
((total_tests = total_tests + 1))
result="PASS"
if [[ $(dpkg-query -W -f='${Status}' "linux-image-generic-hwe-${release}" 2> /dev/null | grep -c "ok installed" || true) != "1" ]]; then
  ((failed_tests = failed_tests + 1))
  result="FAIL"
fi
...
((total_tests = total_tests + 1))
result="PASS"
if [[ $(lsblk -dn -oNAME,RO | awk '/0$/ {print $1}' | grep -cv '^sr' || true) != "3" ]]; then
  ((failed_tests = failed_tests + 1))
  result="FAIL"
fi
...
((total_tests = total_tests + 1))
result="PASS"
if [[ $(dmsetup deps -o devname vg_data-lv_home | sed -n -e 's/.*(\([^)]*\).*/\1/p' || true) != sdc* ]]; then
  ((failed_tests = failed_tests + 1))
  result="FAIL"
fi
