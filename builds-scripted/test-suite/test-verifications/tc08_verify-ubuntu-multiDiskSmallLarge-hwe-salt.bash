...
# Verify the kernel is hwe
release=$(lsb_release -r -s)
((total_tests=total_tests+1))
result="PASS"
if [[ $(dpkg-query -W -f='${Status}' "linux-image-generic-hwe-${release}" 2>/dev/null | grep -c "ok installed" || true) != "1" ]]
then
  ((failed_tests=failed_tests+1))
  result="FAIL"
fi
echo "${result}: HWE Kernel installed" | tee -a "${TEST_FILE}"
...
# Validate root was installed on /dev/sda (smallest)
((total_tests=total_tests+1))
result="PASS"
if [[ $(df --output=source / | tail -n1 || true) != /dev/sda* ]]
then
  ((failed_tests=failed_tests+1))
  result="FAIL"
fi
...
# Validate home was installed on /dev/sdc (largest)
((total_tests=total_tests+1))
result="PASS"
if [[ $(dmsetup deps -o devname vg_data-lv_home | sed -n -e 's/.*(\([^)]*\).*/\1/p' || true) != sdc* ]]
then
  ((failed_tests=failed_tests+1))
  result="FAIL"
fi
echo "${result}: /dev/sdc was used for /home installation" | tee -a "${TEST_FILE}"
...
((total_tests=total_tests+1))
result="PASS"
if [[ $(dpkg-query -W -f='${Status}' "salt-minion" 2>/dev/null | grep -c "ok installed" || true) != "1" ]]
then
  ((failed_tests=failed_tests+1))
  result="FAIL"
fi
