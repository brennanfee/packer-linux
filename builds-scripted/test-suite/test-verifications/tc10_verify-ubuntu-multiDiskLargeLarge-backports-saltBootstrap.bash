...
((total_tests=total_tests+1))
result="PASS"
if [[ $(df --output=source / | tail -n1 || true) != /dev/sdc* ]]
then
  ((failed_tests=failed_tests+1))
  result="FAIL"
fi
...
# Validate home was installed on /dev/sdb (next largest)
((total_tests=total_tests+1))
result="PASS"
if [[ $(dmsetup deps -o devname vg_data-lv_home | sed -n -e 's/.*(\([^)]*\).*/\1/p' || true) != sdb* ]]
then
  ((failed_tests=failed_tests+1))
  result="FAIL"
fi
...
# Verify the saltstack bootstrap script was downloaded
((total_tests=total_tests+1))
result="PASS"
if [[ ! -f "/usr/local/src/install_salt.sh" ]]
then
  ((failed_tests=failed_tests+1))
  result="FAIL"
fi
echo "${result}: Salt bootstrap script exists" | tee -a "${TEST_FILE}"
